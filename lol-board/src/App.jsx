...
/**
 * LoL Tactical Board — tours avec vision + toggle + édition
 * - Image locale: /sr.jpg (dans /public)
 * - Masques (optionnels): /masks/walls.png /masks/brush.png
 * - Board carré & responsive
 * - Clic sur une tour => active/désactive vision
 * - Mode "Éditer les tours" => drag pour repositionner + sauvegarde (localStorage)
 */

const MAX_BOARD = 1100;
const LSK_TOWERS = "lolboard_towers_v1";

// Vision rayons (px sur le board)
const wardRadius = { stealth: 260, control: 300, trap: 220 };
const tokenVisionRadius = 380;
const towerVisionRadius = 750; // ajuste si besoin

// Positions par défaut des tours — coordonnées NORMALISÉES (0..1) pour être indépendantes de la taille.
// NB: Ce sont des positions *approx*, utilise le mode "Éditer les tours" pour recaler pile à ta carte.
const defaultTowersNormalized = [
  // BLUE lane bot (3), inhib (1), nexus (2)
...
// sampler de pixel (0..255, canal R)
...
export default function TacticalBoard() {
  ...
  // masques LOS (optionnels)
  const wallsImg = useImage("/masks/walls.png");
  const brushImg = useImage("/masks/brush.png");
  const sampleWalls = useSampler(wallsImg);
  // const sampleBrush = useSampler(brushImg); // utile si tu veux faire des règles d’invisibilité par bush
  ...
  }, [tokens, wards, visionSide, bgUrl, showGrid, boardSize, towers, wallsImg]);
  ...
  function drawFog() {
    ...
    // pas de masques => trous ronds
    if (!sampleWalls) {
      ctx.globalCompositeOperation = "destination-out";
      // tours
      towers.filter(t => t.team === visionSide && t.enabled).forEach(t => {
        const x = t.x * boardSize, y = t.y * boardSize;
        punchCircle(ctx, x, y, towerVisionRadius);
      });
      // tokens
      tokens.filter(t => t.team === visionSide).forEach(t => {
        punchCircle(ctx, t.x, t.y, tokenVisionRadius);
      });
      // wards
      wards.filter(w => w.team === visionSide).forEach(w => {
        punchCircle(ctx, w.x, w.y, wardRadius[w.kind] || 250);
      });
      ctx.globalCompositeOperation = "source-over";
      return;
    }
    ...
    const los = (cx, cy, r, stepDeg = 1.2) => {
      for (let ang = 0; ang < 360; ang += stepDeg) {
        ...
        for (let d = 0; d <= r; d += 6) {
          ...
          if (sampleWalls(x, y, boardSize) > 128) {
            ctx.beginPath();
  ...
  function punchCircle(ctx, x, y, r) {
          ...
          <div ref={containerRef} className="rounded-2xl overflow-hidden bg-slate-800 shadow-2xl">
            <div className="relative" style={{ width: boardSize, height: boardSize }}>
              ...
              >
                ...
                {/* Towers (clic = toggle, drag si edit) */}
                {towers.map((tw) => {
